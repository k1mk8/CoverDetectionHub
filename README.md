...
|[THE WORDS REMAIN THE SAME: COVER DETECTION WITH LYRICS TRANSCRIPTION](https://archives.ismir.net/ismir2021/paper/000089.pdf) | Some authors have other aplications. In this paper, they proposed another one approach, called it *Lyrics-recognition based system and a classic tonal-based system*. The authors used it datasets like Da-Tacos and DALI for cover detection. Moreover, they used a few fusion model: 1) Lyrics recognition state-of-the-art framework obtained in MIREX 2020 and it uses a model *Time Delay Neural Network* (TDNN) trained using the English tracks of the DALI dataset. In the background music preprocessing step *Singing Voice Separation* (SVS). Moreover, the complete framework they used *Mel-Frequency Cepstral Coefficients* (MFCC) 2) To calculate the similarity between pairs of transcripts - String matching 3) Finally, they used Tonal-based cover detection which is called Re-MOVE and it training of dataset part of Da-Tacos. Another one, that interesting is more classic HPCP features for cover detection |
